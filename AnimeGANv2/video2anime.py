...
def cvt2anime_video(video, output, checkpoint_dir, output_format='MP4V'):
    ...
    with tf.Session(config=tfconfig) as sess:
        ...
        print(video,os.path.join(output, vid_name.rsplit('.', 1)[0] + "_AnimeGANv2.mp4"))
        ...
        vid.release()
        video_out.release()
        clip1 = VideoFileClip(video)#读入视频
        clip2 = VideoFileClip(os.path.join(output, vid_name.rsplit('.', 1)[0] + "_AnimeGANv2.mp4"))
        final_clip = clips_array([[clip1,clip2]])#上下拼接
        final_clip.write_videofile('testdata/comparevideo/'+os.path.basename(ckpt_name).split('.')[0]+'.mp4')#保存视频
