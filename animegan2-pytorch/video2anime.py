...
#     os.makedirs(args.output_dir, exist_ok=True)
    ...
    if os.path.splitext(input_file)[-1].lower() in [".mp4", ".flv", ".avi", ".mkv"]:
        vid = cv2.VideoCapture(input_file)
        vid_name=os.path.basename(input_file)
...
if __name__ == '__main__':
    ...
    clip1 = VideoFileClip(args.input_file)#读入视频
    clip2 = VideoFileClip(os.path.join(args.output_dir, os.path.basename(args.input_file)))
    final_clip = clips_array([[clip1,clip2]])#上下拼接
    final_clip.write_videofile('samples/comparevideo/'+os.path.basename(args.checkpoint).split('.')[0]+'.mp4')#保存视频
#     print(args.input_file,os.path.join(args.output_dir, os.path.basename(args.input_file)))